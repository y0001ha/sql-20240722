...
-- 그룸화 (GROUP BY) : 조회 결과에 대해 레코드를 하나 이상의 컬럼으로 그룹화하여 결과를 도출하는 것 
-- 일반적으로 집계함수와 함께 사용됨 
...
SELECT AVG(amount)
FROM sale
GROUP BY employee_number; -- SELECT에 사용하지 않아도 됨 
...
FROM sale
GROUP BY employee_number, date; -- 두개 이상 그룹화 가능 

SELECT employee_number, date
FROM sale
GROUP BY employee_number, date; -- 집계 안해도 됨 = select distinct랑 같은 결과 

-- 필터링 (HAVING) : 그룹화된 결과에 필터 작업을 수행 
-- 주의!! WHERE 절과 사용방법이 비슷하지만 WHERE절은 조회할 때 사용이 되고, HAVING절은 조회 결과에 사용
SELECT 
	AVG(amount) '평균',
	employee_number '사번'
FROM sale
WHERE AVG(amount) <= 110000
GROUP BY employee_number; -- > WHERE 불가능 
